...
import {getUniqItems, isOneDay} from "../utils/common";
...
const calculateBetweenDates = (dateFrom, dateTo) => {
  const result = [];
...
const getUniqColorCount = (tasks, color) => {
  return tasks.filter((task) => task.color === color).length;
};

const renderColorChart = (colorCtx, tasks) => {
  ...
  return new Chart(colorCtx, {
    ...
    data: {
      labels: colors,
      datasets: [{
        data: colors.map((color) => getUniqColorCount(tasks, color)),
